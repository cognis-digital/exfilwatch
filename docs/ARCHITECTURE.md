...
- **rules/analyzers** apply the heuristics shipped in `exfilwatch/core.py`.
- **score** ranks by severity.
- **MCP server** (`exfilwatch mcp`) exposes `scan` for Cognis.Studio agents.
