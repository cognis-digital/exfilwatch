"""Native cognis-connect emit for exfilwatch — forward findings to any platform.

Maps exfilwatch's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    exfilwatch ... --format json | exfilwatch-emit --to stix
    exfilwatch-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('detector', '') + ' - ' + rec.get('summary', '')
